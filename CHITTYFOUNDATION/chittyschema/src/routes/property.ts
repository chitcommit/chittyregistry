...
router.get('/pin/:pin/ownership', async (req, res) => {
  try {
    ...
    // Use mock data if environment variable is set (for development)
    const useMock = process.env.USE_MOCK_DATA === 'true';
    const data = useMock
      ? await fetchOwnershipWithMockData(pin)
      : await fetchOwnershipByPIN(pin);
...
router.post('/pin/batch', async (req, res) => {
  try {
    ...
    const results = await Promise.all(
      pins.map(pin =>
        useMock
          ? fetchOwnershipWithMockData(pin)
          : fetchOwnershipByPIN(pin)
      )
    );
